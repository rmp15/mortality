...
my.data <- data.frame(dat=as.character(dat$year),log.rate=log(dat$rate.adj),log.deaths=log(dat$deaths))
...
wt.image(my.w, n.levels = 250,
	...
	periodlab = "periods (months)", show.date = T, date.format="%Y",timelab = "",
	graphics.reset = F)
