...
setwd("S:\\Projects\\Disease_trends\\US_county\\US national PM and mortality/From My Book Duo - Helen/")
...
################## INCLUDE INCOME DATA ####################
load('archive/code/make_data/outputs/income_with_sc')
...
CPIdf <- read.csv2('archive/code/make_data/data/income/CPI.csv',header = TRUE,sep=',',stringsAsFactors = FALSE)
CPI_base <- subset(CPIdf,year==refyr)$CPI
CPIdf$ratio <- as.numeric(unlist(CPI_base))/as.numeric(unlist(CPIdf$CPI))
income_us_merged <- left_join(income_us_merged,CPIdf,by='year')
income_us_merged$pc_const <- income_us_merged$pc_merged*income_us_merged$ratio
...
load('archive/code/make_data/outputs/povperc_with_sc')
...
# get race perc 
poprace_us_merged <- left_join(pop_race_allage_mc_sum,pop_nchs_allage_mc_sum,by=c('year','fips'))
poprace_us_merged$popraceprop <- poprace_us_merged$poprace/poprace_us_merged$poptot
...
poprace_us_merged <- poprace_us_merged[,c(1,2,5)]
...
load('archive/code/make_data/outputs/edu_with_sc')
...
################# INCLUDE UR DATA ######################
load('archive/code/make_data/outputs/urbanpop_with_sc')
# merge by mc 
daturban_mctag <- combine_mc(daturban,scloc.df)
...
daturban_mc_nchs <- left_join(daturban_mc,pop_us_mc)
daturban_mc_nchs$urban <- daturban_mc_nchs$toturban/daturban_mc_nchs$popsum

# interpolate between years 
daturbaninterp <- data.frame()
for (cty in unique(daturban_mc_nchs$fips)) {
  
  x <- c(2000,2010)
  y <- subset(daturban_mc_nchs,fips==cty)$urban
  xout <- c(2001:2009)
  if (sum(is.na(y))==0 ) {
  ...
  dattemp <- data.frame(fips=rep(cty,17),year=c(1999:2015),urban=numeric(length=17))
...
load('archive/code/make_data/outputs/unemployment_with_sc')
