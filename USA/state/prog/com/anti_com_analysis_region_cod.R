...
dat.national$deaths.inv <- round((max(dat.national$deaths) - dat.national$deaths)/10)

# add max(rate.scaled) - rate.scaled for each age-sex group
test <- ddply(dat.national,.(climate_region,sex,age),summarize,rate.scaled.max=max(rate.scaled))
...
#
#     print(paste0('Working on regional inverse COM method 1 for ',sex.lookup[sex.selected],' ',age.selected))
#
...
#
#     print(paste0('Working on regional inverse COM method 2 for ',sex.lookup[sex.selected],' ',age.selected))
#
#     # take dates as subset
...
#     ifelse(!dir.exists(file.loc.temp), dir.create(file.loc.temp,recursive=TRUE), FALSE)
# saveRDS(dat.frame,paste0(file.loc.temp,'anti_com_',sex.lookup[sex.selected],'_',age.selected,'_',region.selected))
#
#     return(dat.frame)
# }
#
...
#
#     print(paste0('Working on regional inverse COM first period method 2 for ',sex.lookup[sex.selected],' ',age.selected))
#
#     # take dates as subset
...
#
#     print(paste0('Working on regional inverse COM second period method 2 for ',sex.lookup[sex.selected],' ',age.selected))
#
#     # take dates as subset
#     dat.temp <- subset(dat,age==age.selected & sex==sex.selected)
#
...
circular.age.mean.rate.2 <- function(age.selected,sex.selected,region.selected) {
    ...
    print(paste0('Working on regional inverse COM rate method 2 for ',sex.lookup[sex.selected],' ',age.selected))
    ...
    file.loc.temp <- paste0(file.loc.entire,'method_2/')
    ifelse(!dir.exists(file.loc.temp), dir.create(file.loc.temp,recursive=TRUE), FALSE)
    saveRDS(dat.frame,paste0(file.loc.temp,'anti_com_rate_',sex.lookup[sex.selected],'_',age.selected,'_',region.selected,'_',cod.arg))
