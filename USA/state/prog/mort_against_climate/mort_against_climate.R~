...
#ifelse(!dir.exists(paste0(file.loc,'/','months_together')), dir.create(paste0(file.loc,'/','months_together'),recursive=TRUE), FALSE)
#ifelse(!dir.exists(paste0(file.loc,'/','months_separate')), dir.create(paste0(file.loc,'/','months_separate'),recursive=TRUE), FALSE)

# load USA data NEED TO GENERALISE
dat <- readRDS(paste0('../../output/prep_data/datus_state_rates_',year.start.arg,'_2013'))
