...
metric4.arg <- as.character(args[11])
metric5.arg <- as.character(args[12])
year.start.analysis.arg <- as.numeric(args[13])
year.end.analysis.arg <- as.numeric(args[14])
...
file.loc3 <- paste0('~/git/climate/countries/USA/output/metrics_development/',dname.arg,'/',metric3.arg,'_',dname.arg,'/')
file.loc4 <- paste0('~/git/climate/countries/USA/output/metrics_development/',dname.arg,'/',metric4.arg,'_',dname.arg,'/')
file.loc5 <- paste0('~/git/climate/countries/USA/output/metrics_development/',dname.arg,'/',metric5.arg,'_',dname.arg,'/')
...
dat.climate3 <- readRDS(paste0(file.loc3,'state_weighted_summary_',metric3.arg,'_',dname.arg,'_1979_2015.rds'))
dat.climate4 <- readRDS(paste0(file.loc4,'state_weighted_summary_',metric4.arg,'_',dname.arg,'_1979_2015.rds'))
dat.climate5 <- readRDS(paste0(file.loc5,'state_weighted_summary_',metric5.arg,'_',dname.arg,'_1979_2015.rds'))
...
dat.merged <- merge(dat.merged,dat.climate4,by.x=c('sex','age','year','month','fips','leap'),by.y=c('sex','age','year','month','state.fips','leap'),all.x=TRUE)
dat.merged <- merge(dat.merged,dat.climate5,by.x=c('sex','age','year','month','fips','leap'),by.y=c('sex','age','year','month','state.fips','leap'),all.x=TRUE)
...
names(dat.merged)[23]='variable3'
names(dat.merged)[24]='variable4'
names(dat.merged)[25]='variable5'
...
# load inla function
source('../models/INLA/03_spatiotemporal/inla_functions_5var.R')

# input arguments into function to perform inference
mapply(inla.function.climate.5var.faster,age.sel=age.arg,sex.sel=sex.arg,year.start=year.start.analysis.arg,
