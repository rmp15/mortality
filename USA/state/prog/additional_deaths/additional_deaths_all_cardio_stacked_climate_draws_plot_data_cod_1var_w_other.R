...
if(model%in%c('1d','1d2')){
    ...
    p1 = ggplot() +
        geom_bar(data=subset(additional.deaths.summary,sex>0&age<99), aes(x=as.factor(age.long),y=deaths.added.mean,fill=cause.legend), stat='identity') +
    ...
    pdf(paste0(file.loc,country,'_rate_pred_type',model,
        '_',year.start,'_',year.end,'_',dname,'_',metric,'_cardiovascular_respiratory_monthly_contig.pdf'),paper='a4r',height=0,width=0)
    p2 =ggplot() +
        geom_bar(data=subset(additional.deaths.summary.monthly,sex>0&month<99&cause!='Other unintentional injuries'), aes(x=as.factor(month.short),y=deaths.added.mean,fill=cause.legend), stat='identity') +
    ...
    p3 = ggplot() +
        geom_bar(data=subset(additional.deaths.summary,sex>0&age<99&cause!='Other unintentional injuries'), aes(x=as.factor(age.long),y=deaths.added.mean,fill=cause.legend), stat='identity') +
    ...
    additional.deaths.summary.perc$cause = factor(additional.deaths.summary.perc$cause, levels=c('Ischaemic heart disease','Cerebrovascular disease','Chronic obstructive pulmonary disease','Respiratory infections', 'Other cardiovascular diseases', 'Other respiratory diseases'))
    ...
    #     # geom_errorbar(data=subset(additional.deaths.intent.summary.perc,intent=='1. Unintentional'),aes(x=as.factor(age.long),ymax=perc.ul,ymin=perc.ll),width=.3,size=0.5) +
    #     geom_hline(yintercept=0,linetype='dotted') +
    #     xlab('Age group (years)') + ylab('Excess realtive risk associated with 1 degree additional warming') +
    #     # ylim(c(min.plot,max.plot)) +
    #     facet_grid(cause~sex.long) +
    #     scale_y_continuous(labels=scales::percent) +
    #     scale_color_manual(values=colors.cardio[c(1,2,4,5)]) +
    ...
    additional.deaths.summary.perc$cause = gsub('Ischaemic heart disease', 'Ischaemic\nheart disease',additional.deaths.summary.perc$cause)
    ...
    pdf(paste0(file.loc,country,'_rate_pred_type',model,
        '_',year.start,'_',year.end,'_',dname,'_',metric,'_cardiovascular_respiratory_excess_risk_freescale_fast_contig.pdf'),paper='a4r',height=0,width=0)
    ggplot() +
    ...
    additional.deaths.summary.monthly.perc$cause = gsub('Ischaemic heart disease', 'Ischaemic\nheart disease',additional.deaths.summary.monthly.perc$cause)
    ...
    pdf(paste0(file.loc,country,'_rate_pred_type',model,
        '_',year.start,'_',year.end,'_',dname,'_',metric,'_cardiovascular_respiratory_monthly_excess_risk_fast_contig.pdf'),paper='a4r',height=0,width=0)
    ggplot() +
        # geom_bar(data=subset(additional.deaths.summary.perc,sex>0&age<99&!(cause%in%c('5. Assault','6. Intentional\nself-harm'))), aes(x=as.factor(age.long),y=perc.mean,fill=cause), stat='identity') +
    ...
    pdf(paste0(file.loc,country,'_rate_pred_type',model,
        '_',year.start,'_',year.end,'_',dname,'_',metric,'_cardiovascular_respiratory_monthly_excess_risk_freescale_fast_contig.pdf'),paper='a4r',height=0,width=0)
    ggplot() +
        # geom_bar(data=subset(additional.deaths.summary.perc,sex>0&age<99&!(cause%in%c('5. Assault','6. Intentional\nself-harm'))), aes(x=as.factor(age.long),y=perc.mean,fill=cause), stat='identity') +
    ...
    pdf(paste0(file.loc,country,'_rate_pred_type',model,
        '_',year.start,'_',year.end,'_',dname,'_',metric,'_cardiovascular_respiratory_monthly_one_panel_excess_risk_fast_contig.pdf'),paper='a4r',height=0,width=0)
    ggplot() +
...
if(model%in%c('1e')){
    ...
    # state estimates
    additional.deaths.summary.state = readRDS(paste0(file.loc,'additional_deaths_summary_age_state_draws.rds'))
    additional.deaths.summary.monthly.state = readRDS(paste0(file.loc,'additional_deaths_summary_monthly_state_draws.rds'))

    additional.deaths.state = readRDS(paste0(file.loc,'additional_deaths_age_state_draws.rds'))
    ...
    # make for state data by all ages
    dat.mort$deaths.pred <- with(dat.mort,pop.adj*rate.adj)
    dat.national <- ddply(dat.mort,.(fips,year,month,sex),summarize,deaths=sum(deaths),deaths.pred=sum(deaths.pred),pop.adj=sum(pop.adj))
    dat.national$rate.adj <- with(dat.national,deaths.pred/pop.adj)
    dat.national <- dat.national[order(dat.national$sex,dat.national$fips,dat.national$year,dat.national$month),]
    dat.national$cause = 'Cardiopulmonary'

    # take one year
    dat.merged.sub <- subset(dat.national,year==year.end)

    # summarise by age-sex and cause across the year
    dat.year.summary = dat.merged.sub
    # dat.year.summary = ddply(dat.merged.sub,.(sex,age,cause),summarize,deaths=sum(deaths.pred))
    # dat.year.summary = fix_cause_names(dat.year.summary)
    ...
    plot.function.perc.change.deaths <- function(dat, sex.sel) {
        ...
        # plotting
        print(ggplot(data=subset(dat,sex==sex.sel)) +
        geom_point(aes(x=as.factor(code_name),y=perc.mean)) +
        geom_errorbar(aes(x=as.factor(code_name),ymin=perc.ll,ymax=perc.ul), width=0) +
        geom_hline(yintercept=0,linetype='dotted') +
        xlab('State') + ylab('Percentage change in deaths rates associated with a 1°C warmer year') +
        scale_x_discrete(limits=rev(levels(dat$code_name)))+
        scale_y_continuous(labels=scales::percent) +
        coord_flip() +
        facet_wrap(~month.short) +
    ...
    # male output to pdf
    pdf(paste0(file.loc,'perc_change_deaths_men_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'.pdf'),paper='a4r',height=0,width=0)
    plot.function.perc.change.deaths(additional.deaths.summary.perc,1)
    dev.off()

    # female output to pdf
    pdf(paste0(file.loc,'perc_change_deaths_women_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'.pdf'),paper='a4r',height=0,width=0)
    ...
    additional.deaths.summary.perc$DRAWSEQ = NULL
    plot <- merge(USA.df,additional.deaths.summary.perc,by.x=c('STATE_FIPS'),by.y=c('fips'))
    plot <- with(plot, plot[order(sex,month,DRAWSEQ,order),])

    plot.function.month <- function(sex.sel) {

        # find limits for plot
        min.plot <- min(plot$perc.mean)
        max.plot <- max(plot$perc.mean)
        ...
        facet_wrap(~month.short) +
        guides(fill=guide_colorbar(barwidth=30, title='Percentage change in deaths rates\nassociated with a 1°C warmer year')) +
        # ggtitle(paste0(cod.print,' ', age.sel,' ',sex.lookup2[sex.sel],' : ', year.start,'-',year.end)) +
        theme_map() +
        theme(text = element_text(size = 15),legend.position = 'bottom',legend.justification=c(0.5,0),strip.background = element_blank()))
        }
    ...
    # male output to pdf
    pdf(paste0(file.loc,'perc_change_deaths_men_map_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'.pdf'),paper='a4r',height=0,width=0)
    plot.function.month(1)
    dev.off()

    # female output to pdf
    pdf(paste0(file.loc,'perc_change_deaths_women_map_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'.pdf'),paper='a4r',height=0,width=0)
    plot.function.month(2)
    dev.off()
