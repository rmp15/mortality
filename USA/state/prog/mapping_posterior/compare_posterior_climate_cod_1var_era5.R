...
metric.2 <- as.character(args[8])
cause <- as.character(args[9]) ; cause <- gsub('_',' ',cause)
contig <- as.numeric(args[10])
pw.arg = as.numeric(args[11])

# year.start = 1980 ; year.end = 2016 ; year.end.2 = 2017
# country = 'USA' ; model = 10 ; model.2 = 27 ; dname = 't2m' ;
# metric = 'meanc3' ; metric.2 = 'meanc4'
# cause = 'Intentional self-harm'; contig=1
...
if(pw.arg==0){
    if(contig==1){
        if(cause!='AllCause'){
            ...
            dat.2 <- readRDS(paste0('../../data/climate_effects_era5/',dname,'/',metric.2,'/non_pw/type_',model.2,'/parameters/',
            country,'_rate_pred_type',model.2,'_',year.start,'_',year.end.2,'_',dname,'_',metric.2,'_',cause,'_fast_contig'))
        }
        if(cause=='AllCause'){
            dat <- readRDS(paste0('../../data/climate_effects/',dname,'/',metric,'/non_pw/type_',model,'/parameters/'
...
file.loc <- paste0('../../output/compare_posterior_climate_era5/',year.start,'_',year.end,
...
cod.print = ifelse(cause=='AllCause', 'All cause',
        ...
        ifelse(cause=='Intentional self-harm','Suicides', # TO ASK MAJID. 'Suicides' ???
...
pdf(paste0(file.loc,country,'_rate_pred_type',model,
    ...
    ggplot(data=dat.merged) +
        xlab('Original model results') + ylab('New model results') +
