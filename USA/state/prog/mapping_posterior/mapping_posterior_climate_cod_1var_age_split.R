...
if(model %in% c('1e','1f')){
    ...
    plot.function.age <- function(sex.sel,age.sel) {
        ...
        # male output to pdf
        pdf(paste0(file.loc,'climate_month_params_map_men_age_split',age.break,'_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'_',cause,'.pdf'),paper='a4r',height=0,width=0)
        for(i in sort(unique(dat$age))){plot.function.age(1,i)}
        dev.off()
                
        # female output to pdf
        pdf(paste0(file.loc,'climate_month_params_map_women_age_split',age.break,'_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'_',cause,'.pdf'),paper='a4r',height=0,width=0)
    ...
    # function to plot all excess risk on plot with error and reference from national model as comparison
        ...
        # female output to pdf
        pdf(paste0(file.loc,'climate_month_params_excess_risk_w_national_women_age_split',age.break,'_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'_',cause,'.pdf'),paper='a4r',height=0,width=0)
    ...
    # function to plot all excess risk on plot with error
        ...
        # female output to pdf
        pdf(paste0(file.loc,'climate_month_params_excess_risk_women_age_split',age.break,'_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'_',cause,'.pdf'),paper='a4r',height=0,width=0)
    ...
    # function to plot all excess risk on plot with error by state instead
        ...
        # male output to pdf
        pdf(paste0(file.loc,'climate_month_params_by_state_excess_risk_men_age_split',age.break,'_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'_',cause,'.pdf'),paper='a4r',height=0,width=0)
        for(i in sort(unique(dat$age))){plot.function.excess.risk.state(1,i)}
        dev.off()

        # female output to pdf
        pdf(paste0(file.loc,'climate_month_params_by_state_excess_risk_women_age_split',age.break,'_',model,'_',year.start,'_',year.end,'_',dname,'_',metric,'_',cause,'.pdf'),paper='a4r',height=0,width=0)
