...
if(model=='1d'){
    ...
    for (i in seq(length(sex.filter))) {
        for (j in seq(length(age.filter))) {
                ...
                # THIRD CLIMATE VARIABLE
                current.file <- model.current$summary.random$month7
                current.file$age <- age.filter[j] ; current.file$sex <- i
                ...
                for(k in c(1:12)) {
                    # find the exponentiated means and CIs
                    marginal.exp <- inla.tmarginal(function(x) exp(x), model.current$marginals.random$month7[[k]])
                    odds.mean <- inla.emarginal(function(x) x,marginal.exp) - 1
                    odds.ll <- inla.qmarginal(0.025,marginal.exp) - 1
                    odds.ul <- inla.qmarginal(0.975,marginal.exp) - 1
...
file.loc.local <- paste0('~/data/mortality/US/state/climate_effects/',dname,'/3var/',metric,'/non_pw/type_',model,'/parameters/')
ifelse(!dir.exists(file.loc.local), dir.create(file.loc.local, recursive=TRUE), FALSE)
file.loc.git <- paste0('../../data/climate_effects/',dname,'/3var/',metric,'/non_pw/type_',model,'/parameters/')
